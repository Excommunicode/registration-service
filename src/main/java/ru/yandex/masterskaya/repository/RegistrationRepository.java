...
public interface RegistrationRepository extends JpaRepository<Registration, Long> {
    ...
    @Query("""
            SELECT new ru.yandex.masterskaya.dto.EventRegistrationRequestDTO(e.id, e.username, e.email, e.phone, e.eventId)
            FROM Registration e
            WHERE e.id = :id
            """)
    ...
    @Modifying
    @Query(nativeQuery = true,
            value = "DELETE FROM registrations WHERE phone = :phone AND password = :password")
    void deleteByPhoneAndPassword(String phone, String password);
}